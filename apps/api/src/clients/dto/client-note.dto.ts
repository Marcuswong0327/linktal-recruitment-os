...
/** One entry in Client.notes' JSONB timeline. */
export class ClientNoteDto {
  @ApiProperty({ description: 'Stable id, unique within this client’s notes (for edit/delete)' })
  id!: string;

  @ApiProperty({ example: 'Called to confirm the Terms of Business.' })
  content!: string;
...
export class AddClientNoteDto {
  @ApiProperty({ description: 'Note content', example: 'Called to confirm the Terms of Business.' })
  @IsString()
  @IsNotEmpty()
  @MaxLength(5000)
  content!: string;
...
export class UpdateClientNoteDto {
  @ApiProperty({ description: 'New note content', example: 'Called to confirm the Terms of Business.' })
  @IsString()
  @IsNotEmpty()
  @MaxLength(5000)
  content!: string;
...
export class DeleteClientNoteQueryDto {
  @ApiPropertyOptional({
    description:
      "Optimistic concurrency check: the note's own `editedAt ?? timestamp` as last seen by the caller. If it no longer matches, the note was changed by someone else in the meantime and the request is rejected with 409.",
  })
