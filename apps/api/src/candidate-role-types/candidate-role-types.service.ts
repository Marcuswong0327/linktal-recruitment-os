...
export class CandidateRoleTypesService {
  ...
  /**
   * Upsert on `name` — the candidate form's combobox calls this for "Add
   ...
   */
  create(dto: CreateCandidateRoleTypeDto) {
    const name = dto.name.trim();
    return this.prisma.candidateRoleType.upsert({
      where: { name },
      create: { name },
      update: {},
    });
