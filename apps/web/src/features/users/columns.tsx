...
import { type User, type UserRole, userRoleLabels, userRoleTriggerClassName, userRoles } from './schema';
...
const roleOptions = userRoles.map((value) => ({
  ...
  triggerClassName: userRoleTriggerClassName[value],
}));
const statusOptions = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <EnumSelect
            value={user.roleName ?? ''}
            onValueChange={(v) => onRoleChange(user, v as UserRole)}
            options={roleOptions}
            placeholder="No role"
            disabled={disabled}
