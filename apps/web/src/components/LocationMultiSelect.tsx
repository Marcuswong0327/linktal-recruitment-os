...
export function LocationMultiSelect({
  ...
  return (
    ...
    >
      {/* Absolutely-positioned click-catcher sibling + relative Trigger,
          same split as TagMultiSelect — Trigger is a real <button> internally,
          so it can't wrap another one; this catcher instead sits *behind* it
          in a shared relative container, giving the whole field a click
          target while a chip's own X (inside Trigger) still gets first
          claim. Opens by dispatching a real click on the Trigger itself
          (via triggerRef) rather than a controlled `open` prop on
          Combobox.Root — controlling `open` from here used to also leave it
          controlled for every other interaction (typing, selecting, Escape),
          and that extra React-state round-trip landed one render behind Base
          UI's own position sync, flashing the popup at a stale position on
          first open. */}
