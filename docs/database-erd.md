...
Postgres sequence through a shared `display_id(prefix, seq)` function
(`ALTER SEQUENCE ... OWNED BY` in `0_init`, so the sequence drops with its
table; `display_id()` added in `display_id_format`). It pads to 6 digits but
never truncates past that — `lpad(v, greatest(6, length(v)), '0')` — so a
sequence that passes 999,999 widens instead of colliding with an existing row:

| | | | |
|---|---|---|---|
| `CST-000###` | `CLI-000###` | `STK-000###` | `CDD-000###` |
| `TOB-000###` | `CN-000###` | `CDN-000###` | `JR-000###` |
| `JO-000###` | `SUB-000###` | `INT-000###` | `PLC-000###` |

These double as the **import identity** for the 7 tables the workbook importer
writes to (Client, Stakeholder, StakeholderContactHistory, ClientJobResearch,
Candidate, CandidateContactHistory, JobOrder). The source workbook's own ID
columns are entirely empty and its cross-sheet links are written as name +
spreadsheet row number, so each importer assigns `displayId` from a per-tab
ordinal counter — advanced only on a row that actually imports, in sheet order —
and re-imports match on it. Cross-sheet row links resolve through the row->id
registry the target tab's importer builds while assigning displayIds, not by
recomputing a displayId from the row number (see `import-workbook.ts`'s
"Import identity" doc comment). Natural keys can't do this job: 744 candidate
rows share an email with another row, 568 share a mobile, and 31 have
neither — keying on email would silently merge distinct people.

Consequence: inserting, deleting, or newly rejecting a row *before* another row
on the same tab shifts every later row's ordinal — so a re-import after an edit
like that is only safe against empty tables, not layered on top of a prior
numbering. Every bulk/raw load (a re-import, a restore) must be followed by
`pnpm --filter @linktal/api resync:display-ids`, which advances each sequence
to its table's current max — otherwise the next app-created row on that table
collides with one already imported (`P2002`, a generic 500).
